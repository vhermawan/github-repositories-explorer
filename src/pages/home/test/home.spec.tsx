...
describe('Homepage', () => {
	beforeEach(() => {
		...
		(useSearchUsers as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
			...
			isFetching: false,
			promise: Promise.resolve({ items: [], incomplete_results: false, totalCount: 0 }),
		});
		
		(useUserRepositories as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
			data: [],
			isFetching: false,
	...
	it('successfully renders home page with initial state', () => {
		...
		expect(screen.getByText('Search for GitHub users and explore their repositories')).toBeDefined();
	});
